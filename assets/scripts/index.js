    ...
    for (const [item, amount] of Object.entries(items)) {
        ...
        for (const [x, price] of Object.entries(prices)) {
            if (x.includes(item)) {
                ...
                total += amount * price;
                itemsFound.push(`${item} ${formatNo(amount)} x ${formatNo(price)} = ${formatNo(amount * price)} ⏣`);
                itemsList.push(item);
                break;
            }
        }
        if (found == false) {
            itemsNotFound.push(`${item} ${formatNo(amount)} x 0 = 0 ⏣`);
        }
    }
    ...
    if (total > 0) {
        ...
        for (i = 0, j = itemsList.length; i < j; i += chunk) {
            ...
            for (var x = 0; x < temporary.length; x++) {
                command += ` ${items[temporary[x]]} ${temporary[x]}`;
            }
        ...
        var temp = "";
        temp += "# Total value of the item list, " + formatNo(total) + " ⏣\n";
        temp += "# Recipt generated by: https://itsyasiru.github.io/Dank-Trade-Calculator/\n\n";

        for (var i = 0; i < itemsFound.length; i++) {itemsFound[i] = "+ " + itemsFound[i];}
        for (var i = 0; i < itemsNotFound.length; i++) {itemsNotFound[i] = "- " + itemsNotFound[i];}
